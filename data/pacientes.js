...
export async function agregarPaciente(datos) {
    try {
        const respuesta = await axios.post(`${apiUrl}/pacientes/`, datos,{withCredentials : true})
...
export async function actualizarPaciente(id, datos) {
    try {
        const respuesta = await axios.put(`${apiUrl}/pacientes/${id}`, datos,{withCredentials : true})
        return respuesta.data
    } catch (error) {
        console.log(error)
    }
